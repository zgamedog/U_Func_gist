...
public class AndroidPower : MonoBehaviour
{
    ...
    private void OnGUI()
    {       
        elePwr = string.Format($"<size=22>电池总容量{Power.capacity}毫安,电压{Power.voltage}伏</size>");
        eleSts = string.Format($"<size=22>实时电流{e}毫安,实时功率{(int)(e * Power.voltage)},满电量能玩{((Power.capacity / e).ToString("f2"))}小时</size>");
        GUILayout.Label(elePwr);
        GUILayout.Label(eleSts);
    }
